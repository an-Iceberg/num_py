...
def gauss_algorithm(A: Matrix, b: Vector) -> tuple[Matrix, Vector]:
    n = len(A)
    for pivot_row in range(n):
        for row in range(pivot_row + 1, n):
            factor = A[row][pivot_row] / A[pivot_row][pivot_row]
...
def dot(A: Matrix, B: Matrix) -> Matrix:
    ...
    if len(A) != len(B) or len(A[0]) != len(B[0]):
        raise Exception(
            "Incompatible dimensions for A({}) and B({})".format(len(A), len(B))
        )
    if len(A[0]) != len(B[0]):
        raise Exception(
            "Incompatible dimensions for A[0]({}) and B[0]({})".format(
                len(A[0]), len(B[0])
            )
        )
        ...
        for value in row:
            print(
                "{:> {width}.{precision}f} ".format(
                    value, width=precision + 1, precision=precision
                ),
                end="",
            )
        print()
...
def QR(A: Matrix) -> (Matrix, Matrix):
    ...
    for col in range(n - 1):
        a = list(chain(*map(lambda lst: lst[col : col + 1], R)))[
            col:
        ]  # Copies the nth column below the diagonal from the matrix
...
def gauss_seidel(A: Matrix, b: Vector, x: Vector, tolerance: float) -> Vector:
    ...
    while max_δ > tolerance:
        ...
        for i in range(n):  # Iterating over all elements of x
            ...
            new_x = (1 / A[i][i]) * (
                b[i] - Σ
            )  # Calculating new x with Gauss-Seidel formula
            new_δ = abs(x[i] - new_x)
